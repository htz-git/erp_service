...
CREATE TABLE IF NOT EXISTS `audit_log` (
  ...
  `action_type` VARCHAR(50) NOT NULL COMMENT '操作类型：company_onboard/company_disable/company_enable/company_update, user_create/user_delete/user_update/user_disable/user_enable/user_reset_password/user_permission_add/user_permission_remove, seller_create/seller_update/seller_delete/seller_enable/seller_disable 等',
  `target_type` VARCHAR(30) DEFAULT NULL COMMENT '对象类型：company, user, seller',
...
CREATE TABLE IF NOT EXISTS `refund_application` (
  ...
  `order_id` BIGINT NOT NULL COMMENT '订单ID',
  `order_no` VARCHAR(50) NOT NULL COMMENT '订单号',
  `order_item_id` BIGINT DEFAULT NULL COMMENT '订单明细ID，为空表示整单退款',
  `user_id` BIGINT NOT NULL COMMENT '用户ID',
  ...
  KEY `idx_order_id` (`order_id`),
  KEY `idx_order_id_order_item_id` (`order_id`, `order_item_id`),
  KEY `idx_order_no` (`order_no`),
  KEY `idx_user_id` (`user_id`),
...
CREATE TABLE IF NOT EXISTS `inventory` (
  ...
  `sid` BIGINT NOT NULL COMMENT '店铺ID（Seller ID，引用 seller.id）',
  ...
  `update_time` DATETIME DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP COMMENT '更新时间',
  PRIMARY KEY (`id`),
  UNIQUE KEY `uk_zid_sid_sku_id` (`zid`, `sid`, `sku_id`),
  KEY `idx_zid` (`zid`),
  KEY `idx_sid` (`sid`),
  KEY `idx_zid_sid` (`zid`, `sid`),
  KEY `idx_sku_id` (`sku_id`),
  KEY `idx_product_id` (`product_id`)
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci COMMENT='库存表（按公司+店铺+SKU）';
